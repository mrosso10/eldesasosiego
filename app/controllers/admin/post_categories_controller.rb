...
class Admin::PostCategoriesController < ApplicationController
    ...
    respond_to do |format|
      ...
      format.xlsx do
        render xlsx: 'download',
          filename: "#{Admin::PostCategory.nombre_plural.gsub(' ','-').downcase}-#{Date.today}.xlsx"
      end
    end
  end
  ...
  def create
    respond_to do |format|
      if @post_category.save
        format.html { redirect_to @post_category, notice: "#{ Admin::PostCategory.nombre_singular } creadx." }
  ...
  def update
    respond_to do |format|
      if @post_category.save
        format.html { redirect_to @post_category, notice: "#{ Admin::PostCategory.nombre_singular } actualizadx." }
        format.json { render json: @post_category.decorate }
      else
  ...
  private
    ...
    def set_post_category
      ...
      if action_name.in? %w(show new edit)
        @post_category = @post_category.decorate
      end
    end
