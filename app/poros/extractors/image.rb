...
# https://github.com/yob/pdf-reader/blob/master/examples/extract_images.rb
      ...
      def process_page(page, count)
        ...
        xobjects.each do |name, stream|
          ...
          when :Image
            ...
            when :CCITTFaxDecode
              Extractors::Tiff.new(stream).save("temp_images/#{page.number}-#{count}-#{name}.tif")
            when :DCTDecode
              Extractors::Jpg.new(stream).save("temp_images/#{page.number}-#{count}-#{name}.jpg")
            else
              Extractors::Raw.new(stream).save("temp_images/#{page.number}-#{count}-#{name}.tif")
            end
          when :Form
